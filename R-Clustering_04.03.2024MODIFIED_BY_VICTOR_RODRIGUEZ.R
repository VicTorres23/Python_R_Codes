...
#-------------USING THE SECOND MOST SIGNIFICANT GENE INSTEAD---------------
exprs(gset)["7900167",]

barplot(sort(exprs(gset)["7900167",], decreasing=TRUE), cex.names = 0.5, ylim=c(0, 12))

exprs(gset)["7900167", "GSM662904"]
...
#-----USING THE LAST GENES THAT APPEARED USING head(tT)--------------------
...
# every fifth gene
#heatmap(dat[seq(1, nrow(dat), by=5), ], Colv = NA)

#---------------MAPPING ONLY FOR EVERY THIRD GENE---------------------------
# We use cexRow parameter in 0.3 to visualize all of the genes being
# considered in the heatmap and we map the heatmap every third gene.
...
#-------------MODIFYING THE SETS TO THE GENES THAT APPEARED IN THE HEATMAP
# GENERATED IN LINE 243---------------------------------------------------
...
#----CHANGING TO A DIFFERENT METHOD FOR THE CREATION OF THE DENDOGRAM-------
...
#--CREATING HEATMAPS BY UTILIZING DIFFERENT HIERACHAL CLUSTERING METHODS-----
dEu <- function(x) dist(x, method="euclidian")
...
#Here I'm adding another method and adding a parameter to the Minkowski
#method.
dMan <- function(x) dist(x, method="manhattan")
heatmap(dat, distfun = dMan, Colv = NA, main = "Manhattan Distance")
#For the Mikoswki Methods we are experimenting with diffeentes values for
#P parameter, each different value gives us a different heatmap
...
# Here we change the number of clusters to 6.
k <- 6
cl <- kmeans(dat, k)

niceCols <- brewer.pal(k, "Spectral")
# We also change the sample being plotted to see the difference between
# a 2 hour strain against a 12 hour strain.
...
#plot(cl)
#------------CHANGING THE NUMBER OF K-MEDOIDS----------------------------
# Here we change the number of K-Medoids to 6.
k <- 6
cl<-pam(dat, 4)
# Here we also change the sample being plotted to see the difference between
# a 2 hour strain against a 12 hour strain.
plot(dat[,"t2"],dat[,"t12"], col=niceCols[cl$cluster])
#------------------------------------------------------------------------
